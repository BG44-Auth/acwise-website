...
**How to use:** Open the website you want audited in a Chrome tab, open the
Claude for Chrome extension side panel, and paste the whole block below as
your first message. Nothing to fill in first. Let it run through to the end
before interrupting — the later phases depend on the earlier ones. If it
pauses partway through, just say "continue" — a full audit is long.

---

```
You are auditing the website open in your current browser tab. Your job is
